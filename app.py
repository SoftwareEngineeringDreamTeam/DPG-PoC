...
class ROC:
    ...
    def find_circle(self, px: int):
        for i in range(len(self.data)):
            if abs(self.data[i][0] - px) < 10:
                return i
...
with dpg.handler_registry():
    dpg.add_mouse_move_handler(callback=roc.move_circle)
    dpg.add_mouse_down_handler(callback=roc.mouse_down)
    dpg.add_mouse_release_handler(callback=roc.mouse_release)
...
with dpg.window(tag="Primary Window",width=800, height=600):
    ...
    with dpg.group(horizontal=True):
        with dpg.plot(width = 400):
            ...
            dpg.add_line_series(roc.xVal, roc.yVal, label='Data', parent='y_axis')

        with dpg.plot(width = 400):
            dpg.add_plot_legend()
            dpg.add_plot_axis(dpg.mvXAxis, label="x")
            dpg.add_plot_axis(dpg.mvYAxis, label="y", tag="y_axis2")
            dpg.add_line_series(roc.xVal, roc.yVal, label='Data2', parent='y_axis2')
    ...
    for index in range(len(roc.data)):
        indexStr = str(index)
        if roc.data[index][1] == True:
            dpg.draw_circle([roc.data[index][0], 500], radius=10, color=(0, 255, 0), fill=[0, 255, 0], tag=f"circle_{indexStr}")
        else:
            dpg.draw_circle([roc.data[index][0], 500], radius=10, color=(255, 0, 0), fill=[255, 0, 0], tag=f"circle_{indexStr}")
